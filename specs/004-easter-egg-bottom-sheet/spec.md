...
홈 화면의 FAB(Floating Action Button)에서 이스터에그 옵션을 선택하면, 사용자가 이스터에그를 작성할 수 있는 바텀시트가 표시됩니다. 이 바텀시트는 첨부파일 미리보기 기능을 제공하며, 사용자가 직관적으로 이스터에그를 작성하고 다음 단계로 진행할 수 있도록 합니다.
...
- 이스터에그 작성 폼을 직관적으로 제공
- 첨부파일(이미지, 음원, 비디오) 미리보기 기능 제공
...
**설명**: 바텀시트는 컨텐츠에 맞게 자동으로 높이가 조정되며, 드래그 기능은 제공하지 않습니다.

**수락 기준**:
- 바텀시트의 초기 표시 높이는 컨텐츠에 맞게 자동 조정됨
- 바텀시트는 위아래로 드래그할 수 없음
- 컨텐츠가 많을 경우 스크롤 가능 영역 제공

### FR-3: 바텀시트 컨텐츠 구성 (폼 기반)

**설명**: 바텀시트는 Figma 디자인에 따라 이스터에그 작성 폼을 포함합니다.

**수락 기준**:
- 바텀시트 상단에 드래그 핸들이 표시됨
- 제목 "이스터에그 작성"과 설명 "현재 위치에 추억을 숨겨요" 표시
- **제목 입력 필드**: 최대 30자, 글자 수 표시, placeholder: "추억의 제목을 입력하세요"
- **메시지 입력 영역**: 최대 500자, 글자 수 표시, placeholder: "미래의 나에게 또는 친구에게 남길 메시지를 작성하세요..."
- **첨부파일 버튼 3개**: 사진, 음원, 동영상 (각각 @remixicon/react 아이콘 포함)
  - **사진**: 파일 선택 가능, 이미지 미리보기 표시
    ...
    - 이미지 삭제 기능 포함
  - **음원**: 버튼 클릭 시 모달 표시 (직접 녹음 또는 파일 업로드)
    - 모달 Figma 디자인:
    ...
    - 녹음/업로드 완료 후 미리보기 표시
    - 미리보기 Figma 디자인: https://www.figma.com/design/k7IWFISJsHIQ4g6FoAZqup/%ED%83%80%EC%9E%84%EC%BA%A1%EC%8A%90---%EC%9D%B4%EC%8A%A4%ED%84%B0%EC%97%90%EA%B7%B8?node-id=599-5660&m=dev
    - 오디오 플레이어 기능 (재생/일시정지)
    - 음원 삭제 기능 포함
  - **동영상**: 파일 선택 가능, 썸네일 미리보기 표시
...
- 폼 필드 간격과 패딩은 Figma 디자인 스펙을 준수함

### FR-4: 바텀시트 하단 버튼 영역

**설명**: 바텀시트 하단에는 취소와 작성 완료 버튼이 고정되어 표시됩니다.

**수락 기준**:
- 하단에 DualButton 컴포넌트가 고정 배치됨
- 취소 버튼 클릭 시 바텀시트가 닫힘
- "작성 완료" 버튼 클릭 시 폼 데이터로 이스터에그 생성
- **필수 필드 검증**: 제목이 입력되지 않은 경우 버튼 비활성화
- 버튼 영역은 스크롤되지 않고 항상 하단에 고정됨
- 작성 완료 버튼 비활성화 시 회색(#e8e8e8) 배경
...
- 바텀시트는 위아래로 드래그할 수 없음 (드래그 기능 없음)
...
5. 바텀시트에 "이스터에그 작성" 폼이 표시됨
6. 사용자가 제목을 입력함 (필수)
7. 사용자가 메시지를 입력함 (선택)
8. 사용자가 원하는 경우 첨부파일을 추가함 (선택):
   - 사진: 파일 선택 → 미리보기 표시
   - 음원: 모달 열림 → 직접 녹음 또는 파일 업로드 → 오디오 플레이어 표시
   - 동영상: 파일 선택 → 썸네일 미리보기 표시
9. 사용자가 "작성 완료" 버튼을 탭함
10. 바텀시트가 닫히고 입력된 데이터로 이스터에그가 생성됨
...
2. 사용자가 "사진" 버튼을 탭함
3. 이미지 파일을 선택함
4. 선택한 이미지의 미리보기가 바텀시트에 표시됨
5. 사용자가 "음원" 버튼을 탭함
6. 음원 모달이 표시됨
7. 사용자가 직접 녹음하거나 파일을 업로드함
8. 녹음/업로드 완료 후 오디오 플레이어가 포함된 미리보기가 표시됨
9. 사용자가 "동영상" 버튼을 탭함
10. 비디오 파일을 선택함
11. 선택한 비디오의 썸네일 미리보기가 바텀시트에 표시됨
12. 각 미리보기에서 삭제 버튼으로 첨부파일을 제거할 수 있음
...
- 모든 닫기 방법(오버레이, 취소 버튼, ESC)이 정상 작동함
- 첨부파일 미리보기가 올바르게 표시됨
- 음원 모달이 정상적으로 열리고 닫힘
- 오디오 플레이어가 정상적으로 작동함
- 확인 버튼이 필수 필드 입력 상태에 따라 올바르게 활성화/비활성화됨
...
- `height` (number): 현재 바텀시트 높이 (px, 컨텐츠에 맞게 자동 조정)
...
**속성**:
- `title` (string): 이스터에그 제목 (필수, 최대 30자)
- `message` (string): 메시지 내용 (선택, 최대 500자)
- `attachments` (File[]): 첨부 파일 목록 (선택)
  - 사진 (image/*)
  - 음성 (audio/*)
  - 동영상 (video/*)
- `location` (Location): 현재 위치 정보 (자동 수집)
...
- BottomSheet 공통 컴포넌트가 정상 동작함
- DualButton 공통 컴포넌트가 정상 동작함
- 카카오 지도 API가 정상적으로 로드됨
- 브라우저가 파일 선택 및 미디어 API를 지원함
