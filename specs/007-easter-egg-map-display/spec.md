...
### FR-1: 지도에 이스터에그 마커 표시

**설명**: 카카오 지도에 주변 이스터에그의 위치를 마커로 표시합니다.

**수락 기준**:
- 지도가 최초 로드될 때 현재 위치 기준 반경 300m 이내의 이스터에그 목록을 조회하여 마커를 표시함
- 지도 이동 또는 확대/축소 시에는 목록을 재조회하지 않음 (최초 로드 시에만 조회)
- 마커는 이스터에그의 실제 위치(위도, 경도)에 정확하게 표시됨
- 내가 숨긴 이스터에그와 친구가 숨긴 이스터에그를 구분하여 표시할 수 있음 (선택사항)
- 마커 클릭 시 해당 이스터에그 정보 조회 화면으로 이동함
...
2. 지도에 주변 이스터에그 마커가 표시됨
3. 시스템이 현재 위치를 파악하고 주변 이스터에그와의 거리를 자동으로 계산함
4. 친구 이스터에그 중 현재 위치 기준 30m 이내인 것이 있으면 자동으로 발견 성공 모달이 표시됨
5. 시스템이 이스터에그 상세 정보를 조회하여 표시함
6. 사용자가 콘텐츠를 확인함
...
2. 시스템이 실시간으로 현재 위치를 추적하고 주변 이스터에그와의 거리를 계산함
3. 이동 중 30m 이내로 진입한 친구 이스터에그가 있으면 자동으로 발견 성공 모달이 표시됨
4. 시스템이 이스터에그 상세 정보를 조회하여 표시함
5. 사용자가 콘텐츠를 확인함
6. 사용자가 모달을 닫고 지도로 돌아감

### 시나리오 1-2: 마커 클릭을 통한 이스터에그 조회

1. 사용자가 지도에서 특정 마커를 클릭함
2. 시스템이 해당 이스터에그의 기본 정보를 조회함
3. 소유자와 거리 조건에 따라 적절한 모달이 표시됨
4. 사용자가 모달에서 이스터에그 정보를 확인함
5. 사용자가 모달을 닫고 지도로 돌아감

### 시나리오 2: 내 이스터에그 조회

1. 사용자가 지도에서 자신이 숨긴 이스터에그 마커를 클릭함
2. 시스템이 이스터에그 기본 정보를 조회하여 소유자를 확인함
3. 내 이스터에그 UI가 표시됨
4. 시스템이 발견자 목록을 조회하여 표시함
...
### 시나리오 5: 이스터에그 목록 조회 실패

1. 사용자가 홈 화면에 접근하여 카카오 지도를 확인함
2. 시스템이 주변 이스터에그 목록을 조회하려고 시도함
...
1. 사용자가 지도에서 이스터에그 마커를 클릭함
2. 시스템이 이스터에그 기본 정보를 조회하려고 시도함
3. 네트워크 오류 또는 서버 오류로 인해 조회가 실패함
4. 사용자에게 적절한 오류 메시지가 표시됨
5. 모달이 닫히고 지도로 돌아감
6. 사용자가 다시 시도할 수 있음
...
**설명**: 지도에 표시할 이스터에그의 기본 정보

**속성**:
- `capsuleId` (string, 필수): 이스터에그 고유 식별자
- `latitude` (number, 필수): 위도
- `longitude` (number, 필수): 경도
- `ownerInfo` (object, 필수): 소유자 정보 (내 것/친구 것 구분용)
- `accessibility` (object, 선택): 접근 가능 여부 판단용 메타데이터
- `requiresDistanceCheck` (boolean, 선택): 거리 계산 필요 여부 (false인 경우 거리 계산 없이 바로 접근 가능)
...
- 이스터에그 목록 조회 API는 `GET /api/capsules`를 사용함
- 이스터에그 목록 조회는 최초 지도 로드 시에만 수행되며, 현재 위치 기준 반경 300m 이내의 이스터에그만 조회함
- 이스터에그 기본 정보 조회 API는 `GET /api/capsules/{id}`를 사용함
- 이스터에그 상세 정보 조회 API는 `GET /api/capsules/{id}/detail`을 사용함
- 발견 기록 API는 `POST /api/capsules/{id}/viewers`를 사용함
- 발견자 목록 조회 API는 `GET /api/capsules/{id}/viewers`를 사용함
- 거리 계산은 사용자의 현재 위치와 이스터에그 위치 간의 직선 거리를 기준으로 함
...
### EC-2: 이스터에그 목록이 매우 많은 경우

**상황**: 현재 위치 기준 반경 300m 이내에 수백 개의 이스터에그가 있는 경우
...
이 명세서는 다음 API를 사용한다고 가정하지만, 실제 구현 시 정확한 요청/응답 형식은 기술 계획 단계에서 정의됩니다:

- **이스터에그 목록 조회**: `GET /api/capsules`
- **이스터에그 기본 정보 조회**: `GET /api/capsules/{id}`
- **이스터에그 상세 정보 조회**: `GET /api/capsules/{id}/detail`
- **이스터에그 발견 기록**: `POST /api/capsules/{id}/viewers`
- **이스터에그 발견자 목록 조회**: `GET /api/capsules/{id}/viewers`
