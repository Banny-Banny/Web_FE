...
- 이스터에그 기본 정보(제목, 콘텐츠, 미디어 정보 등)를 조회하여 표시함
...
6. 시스템이 캡슐 기본 정보를 조회함
7. 콘텐츠 타입에 맞는 UI로 기본 정보가 표시됨 (제목, 텍스트, 이미지, 음원, 영상 등)
8. 사용자가 콘텐츠를 확인함
...
**설명**: 발견 성공 시 표시할 캡슐의 기본 정보 (GET /api/capsules/{id} 응답 기반)

**속성**:
- `id` (string, 필수): 캡슐 고유 식별자
- `title` (string, 선택): 제목
- `content` (string, 선택): 텍스트 내용
- `media_types` (string[], 선택): 미디어 타입 배열
- `media_urls` (string[], 선택): 미디어 URL 배열
- `media_items` (object[], 선택): 미디어 아이템 배열
- `author` (object, 선택): 작성자 정보
- `viewers` (object[], 선택): 발견자 목록
- `view_limit` (number, 선택): 조회 제한 수
- `view_count` (number, 선택): 현재 조회 수
...
- 캡슐 기본 정보 조회 API는 `GET /api/capsules/{id}`를 사용함 (lat, lng 쿼리 파라미터 필수)
...
**상황**: 발견 성공 모달에서 기본 정보를 조회하는 데 실패한 경우
...
**설명**: 특정 캡슐의 기본 정보를 조회합니다.

**Path 파라미터**:
- `id` (string, 필수): 캡슐 ID (UUID)

**Query 파라미터**:
- `lat` (number, 필수): 사용자 현재 위도
- `lng` (number, 필수): 사용자 현재 경도
...
**설명**: 사용자가 캡슐(이스터에그)를 발견했음을 기록합니다. 첫 발견인 경우 `view_count`를 증가시키고, 중복 발견인 경우 로그만 조회합니다.

**Path 파라미터**:
- `id` (string, 필수): 캡슐 ID (UUID)

**Body 파라미터** (선택):
- `lat` (number, 선택): 발견 위치 위도 (-90~90)
- `lng` (number, 선택): 발견 위치 경도 (-180~180)
...
**에러 응답**:
- `400`: 잘못된 UUID 형식, lat/lng 범위 초과
- `401`: 인증 실패 (JWT 누락/만료)
- `404`: 캡슐 미존재/삭제됨
- `500`: 내부 처리 오류

**특이사항**:
- 중복 방지: `capsule_access_logs` 테이블의 UNIQUE 제약 (capsule_id + viewer_id)
- 본인이 작성한 캡슐도 발견 기록 가능
- lat/lng는 선택 사항 (위치 정보 없이도 발견 기록 가능)

#### 캡슐 발견자 목록 조회 API

**엔드포인트**: `GET /api/capsules/{id}/viewers`

**설명**: 특정 캡슐(이스터에그)를 발견한 사용자 목록을 조회합니다.
...
**특이사항**:
- 정렬: 조회 시각(`viewed_at`) 오름차순 (가장 먼저 발견한 사람이 첫 번째)
- 권한: 인증된 사용자는 누구나 조회 가능 (본인 캡슐 여부와 무관)
- `view_limit`이 0이면 조회 제한 없음 (무제한)
