...
- **EXPO_PUBLIC_KAKAO_MAP_API_KEY**
  - **주의사항**: Next.js에서는 `NEXT_PUBLIC_` 접두사가 필요하지만, 기존 모바일 앱과의 일관성을 위해 `EXPO_PUBLIC_` 사용
  - **처리 방법**: Next.js 설정에서 환경 변수 접근 처리 또는 변수 변환
...
**변경 사항**:
- `EXPO_PUBLIC_KAKAO_MAP_API_KEY` 환경 변수 접근
...
- **카카오 지도 JavaScript API**: 지도 표시 및 기본 기능
  - API 키 필요: `EXPO_PUBLIC_KAKAO_MAP_API_KEY` 또는 `NEXT_PUBLIC_KAKAO_MAP_API_KEY`
...
### 환경 변수 처리 주의사항
- Next.js에서는 클라이언트 접근을 위해 `NEXT_PUBLIC_` 접두사 필요
- 기존 모바일 앱과의 일관성을 위해 `EXPO_PUBLIC_`도 지원
- `.env.local`에 두 가지 형식 모두 설정 권장
- 또는 `next.config.ts`에서 환경 변수 변환 처리
