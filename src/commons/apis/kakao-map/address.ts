...
/**
 * 좌표를 도로명 주소로 변환하는 API 요청 파라미터
 */
export interface Coord2AddressParams {
  /** 경도 (longitude) */
  x: number;
  /** 위도 (latitude) */
  y: number;
...
/**
 * 좌표를 도로명 주소로 변환합니다.
 * 카카오 REST API의 coord2address 엔드포인트를 사용합니다.
 * 
 * @param params 좌표 파라미터 (x: 경도, y: 위도)
 * @returns 도로명 주소 정보 (road_address.address_name)
 * @throws {Error} API 호출 실패 시
 */
export async function getRoadAddressFromCoord(
  params: Coord2AddressParams
): Promise<string | null> {
  try {
    // 좌표 유효성 검증
    if (!params.x || !params.y) {
    ...
    try {
      // API 요청
      const response = await fetch(
        `https://dapi.kakao.com/v2/local/geo/coord2address.json?x=${params.x}&y=${params.y}`,
        {
          method: 'GET',
          headers: {
            Authorization: `KakaoAK ${apiKey}`,
      ...
      const data: Coord2AddressResponse = await response.json();
      ...
      // 첫 번째 결과의 road_address.address_name 반환
      const firstResult = data.documents[0];
      
      // road_address가 있으면 road_address.address_name 사용
      if (firstResult.road_address) {
        return firstResult.road_address.address_name;
      }
      
      // road_address가 없으면 address.address_name 사용 (지번 주소)
